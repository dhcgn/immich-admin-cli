...
- **When the OpenAPI spec's description, enum, or parameter documentation is unclear, ambiguous, or seemingly incomplete**, cross-check the upstream Immich server source directly (DTOs under `immich-app/immich`'s `server/src/dtos/` on GitHub, e.g. `asset-media.dto.ts` for `AssetMediaSize`) to confirm the actual accepted values, defaults, and behavior before implementing. The spec is generated from these DTOs, so they are the ultimate source of truth when the generated description is terse or missing context (e.g. `x-immich-history` deprecation notes on individual enum values, not just whole operations).
- **When a spec parameter or field is a closed set of possible values (a `schema.enum`, e.g. `AssetMediaSize`, `AssetTypeEnum`, `JobName`), never pass a raw/unchecked user-supplied string straight into the generated type.** Implement an explicit guard (e.g. a small `resolveX`/`validateX` helper, or the generated type's own `Valid()` method where its accepted set matches exactly what this operation allows) that rejects any value outside that closed set, with an error message listing the valid options. Then reflect those exact options in the command's CLI help/`Usage` text (and in `README.md`) — the set the CLI advertises and the set it actually accepts must always match. Do not rely on the upstream API to reject invalid values at request time; some invalid values are only individually deprecated for one specific operation (not invalid across the whole enum — see `AssetMediaSize`'s `original`, valid on `download-original`-style endpoints but deprecated specifically on `GET /assets/{id}/thumbnail`), so a single generated `Valid()` check is not always sufficient — validate against the subset that specific operation actually supports.
...
- CI (`.github/workflows/ci.yml`): fmt/build/vet/test + generated-code freshness check on pushes/PRs to `main`.
- Releases (`.github/workflows/release.yml`): a `v*` tag on `main` produces a stable release. Binaries are cross-compiled for linux/darwin/windows.
...
Flow: **feature branch → PR into `main` + tag (stable)**.

Rules:

- **Branch from `main`.** Feature branches are named `feature/<name>` and deleted at merge; `main` is permanent.
- **The tag makes the stable release, not the merge.** Merging to `main` only runs CI; pushing a `v*` tag on `main` triggers the Release workflow.
...
### 1. Start a feature (from fresh `main`)
