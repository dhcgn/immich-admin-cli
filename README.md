...
|:------:|---------|-------------|
| ⏳ planned | `client-workflow replace-asset` | Replace an existing asset with a new file, keeping its metadata |
...
3. **Copy metadata** from the old asset — favorite, description, rating, capture date, album memberships, tags
4. **Remove** the old asset (to trash by default; `--force` for permanent deletion)
...
<!-- API-TABLE:BEGIN -->
**3 of 235 endpoints implemented** (17 deprecated and 2 internal endpoints omitted per project policy).
...
<details>
<summary><b>Assets</b> (2/24)</summary>
